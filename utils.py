...
CORRECCIONES v2:
  - apply_theme(): st.html() → st.markdown(unsafe_allow_html=True)
    st.html() crea un <iframe> aislado en Streamlit ≥1.36; el CSS dentro
    NUNCA alcanza el DOM padre, causando:
      • Iconos Material mostrando texto literal "keyboard_double_arrow_left"
        (la regla .stApp span {font-family:Outfit} llegaba parcialmente en
        versiones mixtas y destruía la fuente de iconos)
      • Sidebar toggle móvil sin efecto
      • Layout móvil sin estilos correctivos
  - Regla de span corregida: excluye clases material-symbols/material-icons
  - Eliminados selectores CSS data-key (atributo que Streamlit NO renderiza
    en los elementos <button>)
  - CSS móvil mejorado: landscape, sidebar overlay, header scroll horizontal
"""
...
/* ══════════════════════════════════════════
   TIPOGRAFÍA
   FIX: .stApp span ya NO incluye clases material-symbols ni material-icons
   para no destruir los iconos de Streamlit (que se renderizan como texto
   cuando se les cambia la font-family).
══════════════════════════════════════════ */
h1, h2, h3, h4, h5, h6,
...
.sidebar-brand-sub {
  ...
  margin-top: .3rem; letter-spacing: .04em; text-transform: uppercase;
}
.sidebar-section {
  font-family: 'Outfit', sans-serif;
...
[data-testid="stSidebar"] .stButton > button {
  ...
  min-height: 40px !important; font-size: .83rem !important;
  font-weight: 600 !important; text-align: left !important;
  padding: .4rem .75rem !important; border: 1px solid transparent !important;
...
.header-breadcrumb {
  ...
  color: #475569; padding: .25rem .6rem;
  background: rgba(255,255,255,.04); border-radius: 8px;
  border: 1px solid rgba(255,255,255,.06); display: inline-block;
}
...
.hero-banner {
  ...
  border-radius: 20px; padding: 1.5rem 1.75rem; margin-bottom: 1.25rem;
...
.hero-eyebrow {
  ...
  color: #3b82f6; text-transform: uppercase; letter-spacing: .1em; margin-bottom: .5rem;
}
.hero-title {
  font-family: 'Outfit', sans-serif; font-size: 1.55rem; font-weight: 800;
  color: #f8fafc; letter-spacing: -.03em; line-height: 1.15; margin-bottom: .4rem;
}
.hero-subtitle {
  font-family: 'Outfit', sans-serif; font-size: .88rem;
  color: #64748b; font-weight: 400; line-height: 1.5;
}
...
.kpi-card {
  ...
  padding: 1.1rem 1.2rem; min-height: 108px;
  position: relative; overflow: hidden;
...
.kpi-icon      { font-size: 1.1rem; margin-bottom: .4rem; opacity: .8; display: block; }
.kpi-label {
  font-family: 'Outfit', sans-serif; font-size: .72rem; font-weight: 700;
  color: #475569; text-transform: uppercase; letter-spacing: .08em; margin-bottom: .35rem;
}
.kpi-value {
  font-family: 'Outfit', sans-serif; font-size: 1.65rem; font-weight: 800;
  color: #f1f5f9; letter-spacing: -.03em; line-height: 1; margin-bottom: .3rem;
}
.kpi-delta            { font-family: 'Outfit', sans-serif; font-size: .78rem; font-weight: 600; color: #64748b; }
...
.section-title {
  ...
  color: #f1f5f9; letter-spacing: -.01em; margin-bottom: .2rem;
}
.section-subtitle {
  font-family: 'Outfit', sans-serif; font-size: .8rem;
  color: #475569; margin-bottom: .9rem;
}
...
.form-section {
  ...
  border-radius: 16px; padding: 1.1rem 1.2rem 1rem 1.2rem; margin-bottom: .85rem;
}
.form-section-header {
  font-family: 'Outfit', sans-serif; font-size: .72rem; font-weight: 700;
  color: #3b82f6; text-transform: uppercase; letter-spacing: .1em;
...
div[data-testid="stNumberInput"] input {
  ...
  font-weight: 500 !important; background: rgba(15,23,42,.8) !important;
  border: 1.5px solid rgba(255,255,255,.1) !important; border-radius: 12px !important;
  color: #e2e8f0 !important; padding: .55rem .9rem !important;
...
div[data-testid="stTextInput"] input:disabled { opacity: .5 !important; cursor: not-allowed !important; }

div[data-testid="stSelectbox"] > div > div {
  font-family: 'Outfit', sans-serif !important; font-size: .9rem !important;
  background: rgba(15,23,42,.8) !important; border: 1.5px solid rgba(255,255,255,.1) !important;
...
div[data-testid="stDateInput"] input {
  font-family: 'Outfit', sans-serif !important; background: rgba(15,23,42,.8) !important;
  border: 1.5px solid rgba(255,255,255,.1) !important; border-radius: 12px !important;
  color: #e2e8f0 !important;
}
...
/* ══════════════════════════════════════════
   BOTONES
   FIX: eliminados selectores data-key (no existen en el DOM de Streamlit)
══════════════════════════════════════════ */
.stButton > button {
...
/* Fuente solo en texto del botón; NO tocar spans de iconos Material */
.stButton > button p {
  font-family: 'Outfit', sans-serif !important;
}
...
.stButton > button[kind="primary"] {
  ...
  border: 1px solid rgba(96,165,250,.3) !important; color: white !important;
...
.stButton > button[kind="secondary"] {
  ...
  border: 1.5px solid rgba(255,255,255,.1) !important; color: #cbd5e1 !important;
}
.stButton > button[kind="secondary"]:hover {
  background: rgba(51,65,85,.8) !important;
  border-color: rgba(255,255,255,.18) !important; color: #f1f5f9 !important;
}
...
div[data-testid="stDataFrame"] iframe { border-radius: 14px !important; }

/* ══════════════════════════════════════════
   EXPANDERS
══════════════════════════════════════════ */
div[data-testid="stExpander"] {
  background: rgba(15,23,42,.6) !important; border: 1px solid rgba(255,255,255,.07) !important;
  border-radius: 14px !important; overflow: hidden !important; margin-bottom: .5rem !important;
}
div[data-testid="stExpander"] summary {
  font-weight: 600 !important; color: #94a3b8 !important;
  font-size: .88rem !important; padding: .75rem 1rem !important;
}
...
div[data-testid="stTabs"] [role="tablist"] {
  border-bottom: 1px solid rgba(255,255,255,.07) !important; gap: .25rem !important;
}
div[data-testid="stTabs"] button[role="tab"] {
  font-family: 'Outfit', sans-serif !important; font-size: .84rem !important;
...
.pill {
  ...
  padding: .2rem .6rem; border-radius: 999px; font-family: 'Outfit', sans-serif;
...
/* ══════════════════════════════════════════
   DIVIDERS
══════════════════════════════════════════ */
hr { border: none !important; border-top: 1px solid rgba(255,255,255,.06) !important; margin: 1rem 0 !important; }

/* ══════════════════════════════════════════
   QUICK ACCESS SHORTCUTS
══════════════════════════════════════════ */
.shortcut-grid { display: grid; grid-template-columns: 1fr 1fr; gap: .5rem; }
.shortcut-btn {
  background: rgba(15,23,42,.8); border: 1.5px solid rgba(255,255,255,.07);
  border-radius: 14px; padding: .75rem 1rem;
  display: flex; align-items: center; gap: .6rem;
  cursor: pointer; transition: all .15s ease; text-decoration: none;
}
.shortcut-btn:hover {
  background: rgba(37,99,235,.12); border-color: rgba(59,130,246,.25); transform: translateY(-1px);
}
.shortcut-icon  { font-size: 1.2rem; }
.shortcut-label { font-family: 'Outfit', sans-serif; font-size: .84rem; font-weight: 600; color: #cbd5e1; }

/* ══════════════════════════════════════════
   HEADER BOTONES NAV (por posición, sin data-key que no existe en Streamlit)
══════════════════════════════════════════ */
/* Botón toggle sidebar (primer botón del primer row de columns) */
div[data-testid="stColumns"]:first-of-type > div[data-testid="stColumn"]:first-child .stButton > button,
div[data-testid="stHorizontalBlock"]:first-of-type > div[data-testid="column"]:first-child .stButton > button {
  min-height: 36px !important; border-radius: 10px !important; font-size: 1rem !important;
}
...
@media (max-width: 768px) {
  ...
  /* Header: scroll horizontal para que no se apile verticalmente */
  ...
  div[data-testid="stColumns"]:first-of-type::-webkit-scrollbar,
  div[data-testid="stHorizontalBlock"]:first-of-type::-webkit-scrollbar { display: none !important; }

  div[data-testid="stColumns"]:first-of-type > div[data-testid="stColumn"],
  div[data-testid="stHorizontalBlock"]:first-of-type > div[data-testid="column"] {
    min-width: fit-content !important; flex: 0 0 auto !important;
  }

  /* Botones nav del header: más compactos en móvil */
  div[data-testid="stColumns"]:first-of-type .stButton > button,
  div[data-testid="stHorizontalBlock"]:first-of-type .stButton > button {
    font-size: .72rem !important; min-height: 34px !important;
    padding: .25rem .5rem !important; border-radius: 9px !important;
    white-space: nowrap !important;
  }
  ...
  .app-header-title { font-size: .88rem; }
}
...
@media (max-height: 500px) and (orientation: landscape) {
  .block-container { padding: .3rem .6rem 2rem .6rem !important; }
  .hero-banner     { padding: .75rem 1rem; }
  .hero-title      { font-size: 1rem; }
  .kpi-card        { min-height: 70px; padding: .5rem .7rem; }
  .kpi-value       { font-size: 1.1rem; }
  /* Header más compacto en landscape */
  div[data-testid="stColumns"]:first-of-type .stButton > button,
  div[data-testid="stHorizontalBlock"]:first-of-type .stButton > button {
    font-size: .68rem !important; min-height: 30px !important;
...
def make_quote_pdf(quote_number, quote_date, client_row, vendor_name,
    ...
    if logo_path:
        try:
            c.drawImage(str(logo_path), 42, height - 66, width=150, height=32,
    ...
    c.setFont("Helvetica", 10)
    left_lines  = [str(client_row.get("name","")), str(client_row.get("address","") or ""),
                   str(client_row.get("phone","") or ""), str(client_row.get("email","") or "")]
    right_lines = ["Abaroa Smart", f"Ejecutivo: {vendor_name or 'Abaroa Smart'}",
                   "Osorno, Región de Los Lagos", "contacto@abaroasmart.com"]
    yl = y - 18
    ...
    table_w, table_x = 500, (width - 500) / 2
    table_top, col_widths, row_h = height - 314, [250, 70, 90, 90], 26
    ...
    if notes:
        for line in str(notes).splitlines()[:2]:
            if line.strip():
                footer_lines.append(f"• {line.strip()}")
...
def make_project_delivery_pdf(project_id):
    ...
    items = conn.execute(
        "SELECT * FROM project_items WHERE project_id=? AND item_type IN ('producto','kit_component','insumo') ORDER BY id",
        (project_id,)
    ).fetchall()
    ...
    c.drawCentredString(width / 2, height - 118, "Acta de Entrega")

    y = height - 175
    c.setFont("Helvetica-Bold", 12)
    c.drawString(50, y, "Datos del Cliente"); c.drawString(width / 2 + 20, y, "Datos del Proyecto")
    c.setFont("Helvetica", 10)
    left  = [str(project["client_name"] or ""), str(project["client_address"] or ""),
             str(project["client_phone"] or ""),  str(project["client_email"] or "")]
    right = [f"N° Proyecto: {project['project_number'] or '-'}",
             f"Cotización: {project['quote_number'] or '-'}",
             f"Instalación: {project['installation_date'] or '-'}",
             f"Entrega: {project['delivery_date'] or '-'}"]
    yl = y - 18
    ...
    table_w, table_x = 500, (width - 500) / 2
    table_top, col_w, row_h = height - 310, [280, 70, 70, 80], 24
    ...
    ycl = yrow - 20
    c.setFont("Helvetica-Bold", 11); c.setFillColor(navy); c.drawString(50, ycl, "Checklist de entrega")
    ...
    c.line(width - 220, sig_y + 20, width - 50, sig_y + 20)
    c.drawString(width - 220, sig_y + 6, "Firma técnico Abaroa Smart")
...
def make_pdf(title, subtitle="", sections=None):
    ...
    logo_path = _pdf_logo_path()
    if logo_path:
        try: c.drawImage(str(logo_path), 40, y - 35, width=140, height=35,
                         preserveAspectRatio=True, mask="auto")
