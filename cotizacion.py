...
from database import (
    ...
    get_quote_stock_warnings, IVA_RATE, link_survey_to_quote,
)
from utils import money, logo, make_quote_pdf, whatsapp_share_link, send_quote_email
...
def render():
    ...
    prefill_id = st.session_state.get("prefill_quote_client_id")
    if prefill_id:
        match = clients_df[clients_df["id"] == prefill_id]
        if not match.empty:
            try:
                default_client_idx = client_names.index(match.iloc[0]["name"])
            except ValueError:
                default_client_idx = 0
    client_name = c1.selectbox("Cliente", client_names, index=default_client_idx)
    ...
    if g2.button("💾 Guardar cotización", use_container_width=True):
        ...
        if errors:
            ...
            pending_survey = st.session_state.pop("pending_survey_link", None)
            if pending_survey:
                link_survey_to_quote(pending_survey, quote_id)
            st.session_state.pop("prefill_quote_client_id", None)
    ...
    # ── Envío al cliente ─────────────────────────────────────────────────────
    st.markdown("#### Enviar al cliente")
    s1, s2 = st.columns(2)
    with s1:
        wa_msg = (f"Hola {client_row.get('name','')}, te comparto la cotización "
                  f"{quote_number} por un total de {money(total)}. "
                  f"Cualquier duda, escríbeme por acá.")
        wa_link = whatsapp_share_link(client_row.get("phone", ""), wa_msg)
        if wa_link:
            st.link_button("💬 Abrir WhatsApp (adjunta el PDF manualmente)", wa_link, use_container_width=True)
        else:
            st.caption("Cliente sin teléfono registrado — no se puede generar el link de WhatsApp.")
    with s2:
        if st.button("📧 Enviar por correo", use_container_width=True):
            client_email = client_row.get("email", "")
            body = (f"Hola {client_row.get('name','')},\n\n"
                    f"Te adjuntamos la cotización {quote_number} por un total de {money(total)}.\n\n"
                    f"Saludos,\nAbaroa Smart")
            ok, msg = send_quote_email(
                client_email, f"Cotización {quote_number} — Abaroa Smart",
                body, current_quote_pdf, f"{quote_number}.pdf",
            )
