...
STEPS = [("cliente","Cliente"),("levantamiento","Levantamiento"),("cotizacion","Cotización"),("proyecto","Proyecto"),("ot","OT"),("cierre","Acta / Cierre")]
    ...
    with c1:
        if step == "cliente":
            ...
            if clients_df.empty:
                ...
                if st.button("Continuar a Levantamiento →", type="primary"):
                    st.session_state["workflow_client_id"] = cli_id
                    _go("levantamiento"); st.rerun()

        elif step == "levantamiento":
            st.markdown("### 2) Levantamiento en terreno")
            client_id = st.session_state.get("workflow_client_id")
            if not client_id:
                _go("cliente"); st.rerun()
            st.info("Registra los ítems detectados en la visita (cámaras, interruptores, cajas de registro, etc.) "
                    "en el módulo de Levantamiento. Cuando termines, esos ítems se pueden trasladar directo a la cotización.")
            lv1, lv2, lv3 = st.columns(3)
            if lv1.button("Ir a módulo Levantamiento", type="primary"):
                st.session_state["current_tab"] = "Levantamiento"; st.rerun()
            if lv2.button("Continuar a Cotización →"):
                _go("cotizacion"); st.rerun()
            if lv3.button("← Volver"):
                _go("cliente"); st.rerun()
